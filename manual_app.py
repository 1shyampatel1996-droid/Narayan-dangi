...
st.title("📊 Manual ITM Signal & Price Dashboard (Demo)")
...
st.sidebar.markdown("### Nifty Group")
manual_nifty_spot = st.sidebar.number_input("Nifty 50 Open", value=22000.0, step50=50.0)
manual_nifty_fut = st.sidebar.number_input("Nifty Future Open", value=22050.0, step50=50.0)

st.sidebar.markdown("### Sensex Group")
...
# निफ्टी का स्ट्राइक अंतर 50 का होता है, सेंसेक्स का 100 का
def get_itm_recommendation(price, color_status, is_nifty=True):
    step = 50 if is_nifty else 100
    # यदि ग्रीन है तो Call (CE) - नीचे का स्ट्राइक या नियरेस्ट ITM
    # यदि रेड है तो Put (PE)
    base_strike = round(price / step) * step
    
    if color_status == "green":
        # Call के लिए ITM आमतौर पर स्पॉट से थोड़ा नीचे या बराबर होता है
        itm_strike = base_strike - step if price < base_strike else base_strike
        return f"🟢 ITM Call (CE): {int(itm_strike)} CE"
    else:
        # Put के लिए ITM आमतौर पर स्पॉट से थोड़ा ऊपर या बराबर होता है
        itm_strike = base_strike + step if price > base_strike else base_strike
...
# ITM रिकमेंडेशन निकालें (प्राइस कलर के आधार पर)
n_spot_itm = get_itm_recommendation(manual_nifty_spot, n_price_spot_col, True)
n_fut_itm = get_itm_recommendation(manual_nifty_fut, n_price_fut_col, True)

s_spot_itm = get_itm_recommendation(manual_sensex_spot, s_price_spot_col, False)
s_fut_itm = get_itm_recommendation(manual_sensex_fut, s_price_fut_col, False)
...
<div style="display: flex; justify-content: space-between; padding: 4px 0; font-size: 16px;">
    ...
    <span style="width: 15%; text-align: right; color: {s_digit_fut_col}; font-weight: bold;">Digit: {s_fut_third}</span>
